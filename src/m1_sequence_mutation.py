...
def run_test_zero_changer():
    ...
    # Test 2:
    test1 = ([0, 0], [0, 4, 5, 6, 7, 10], [1, 2, 3, 0, 5, 6], [11, 10, 9])
    expected1 = ([1, 2], [3, 4, 5, 6, 7, 10], [1, 2, 3, 4, 5, 6], [11, 10, 9])
    zero_changer(test1)
    print()
    print('Test 1:')
    print('  Expected:', expected1)
...
def zero_changer(tuple_of_lists):
    ...
    for k in range(len(tuple_of_lists)):
        for n in range(len(tuple_of_lists[k])):
            if tuple_of_lists[k][n] == 0:
                tuple_of_lists[k][n] = count
                count = count + 1
